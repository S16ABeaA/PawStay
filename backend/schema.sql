...
create table if not exists bookings (
  ...
  med_cert_url    text,                 -- optional medical certificate upload
  vaccine_record_url text,              -- vaccine record upload
...
-- ============================================================
-- ATOMIC BOOKING FUNCTION  (concurrency-safe with advisory locks)
-- ============================================================
...
begin
  ...
  if v_checkout is not null then
    v_cur := v_checkin;
    -- Acquire an advisory lock for every night (blocks concurrent txns on same slot)
    ...
    -- Now check availability for each night
    v_cur := v_checkin;
    while v_cur < v_checkout loop
      v_next_day := v_cur + 1;
  ...
  -- ==========================================================
  -- INSERT  (we are still inside the same transaction + lock)
  -- ==========================================================
...
-- ============================================================
-- JOB LOCKS  (distributed background-job coordination)
-- ============================================================
-- Prevents duplicate execution of scheduled background jobs when
-- multiple server instances run simultaneously.  The backend service
-- role bypasses RLS, so no row-level policies are needed.
-- ============================================================
