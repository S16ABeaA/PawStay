  ...
  useEffect(() => {
    const loadingInterval = setInterval(() => {
      setProgress((prev) => {
        ...
        if (next > 30 && next < 60) setLoadingText("Sniffing out the best stays");
        if (next >= 60 && next < 90) setLoadingText("Grooming the details");
        if (next >= 90) setLoadingText("Almost ready to roll over");
