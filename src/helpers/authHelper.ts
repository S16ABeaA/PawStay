...
const resolveRequestOrigin = (url: string): string => {
  try {
    return new URL(resolveApiUrl(url)).origin;
  } catch {
...
export const authHelper = {
  fetchWithAuth: async (url: string, options: RequestInit = {}) => {
    ...
    const isApi = isApiEndpoint(resolvedUrl);
    const shouldIncludeCredentials = isAuthenticatedEndpoint(resolvedUrl, options.method);
    ...
    let res = await fetch(resolvedUrl, { ...options, credentials, cache: "no-store", headers });
    ...
    if (isApi && mutating && res.status === 403) {
      ...
      res = await fetch(resolvedUrl, { ...options, credentials, cache: "no-store", headers: retryHeaders });
    }
